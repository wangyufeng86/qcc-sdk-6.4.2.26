...
#if HAVE_NUMBER_OF_BITSERIALS > 0
        case BITSERIAL_INSTANCE_0:
            return (uint16)(hal_get_reg_bitserial0_rwb() & 0x3);
#endif
#if HAVE_NUMBER_OF_BITSERIALS > 1
        case BITSERIAL_INSTANCE_1:
            return (uint16)(hal_get_reg_bitserial1_rwb() & 0x3);
#endif
        default:
            panic(PANIC_BITSERIAL_HAL_ERROR); /* Does not return */
    }
...
#if HAVE_NUMBER_OF_BITSERIALS > 0
        case BITSERIAL_INSTANCE_0:
            hal_set_reg_bitserial0_set_sel_inactive(v);
            break;
#endif
#if HAVE_NUMBER_OF_BITSERIALS > 1
        case BITSERIAL_INSTANCE_1:
            hal_set_reg_bitserial1_set_sel_inactive(v);
            break;
#endif
        default:
            panic(PANIC_BITSERIAL_HAL_ERROR); /* Does not return */
