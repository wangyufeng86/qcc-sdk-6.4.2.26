...
/*!
    @brief Secure Connection Override actions.
*/
typedef enum {
    sc_override_action_disable  = 0x00,     /*!< Disable SC, if enabled. */
    sc_override_action_enable   = 0x01,     /*!< Enable SC, if disabled. */
    sc_override_action_delete   = 0xFF      /*!< Delete existing override. */
} sc_override_action_t;
...
typedef enum
{
    ...
    CL_DM_SECURE_CONNECTIONS_OVERRIDE_CFM,
...
/*!
    @brief Response to ConnectionDmSecureConnectionsOverrideReq(), indicates
...
typedef struct
{
    ...
    hci_status              status;
    /*! The BR/EDR Public address for the override. */
    bdaddr                  addr;
    /*! The override action requested. */
    sc_override_action_t    override_action;
} CL_DM_SECURE_CONNECTIONS_OVERRIDE_CFM_T;

/*!
    @brief Response to ConnectionDmSecureConnectionsMaxBdaddrReq(), indicates
    the status of that request and the maximum number of remote device
    BDADDR that can have their Secure Connections behaviour overrriden.
*/
typedef struct
{
    /*! Status of the request. */
    hci_status              status;
    /*! Maximum number of BDADDR that can be overridden. */
    uint8                   max_override_bdaddr;
} CL_DM_SECURE_CONNECTIONS_OVERRIDE_MAX_BDADDR_CFM_T;
...
/*****************************************************************************
 * Enable/Disable Secure Connections per BDADDR
 *
 * The following functions can be used to enable Secure Connections during
 * pairing to a specific BDADDR, if Secure Connections are not enabled by
 * default OR to disable Secure Connections to a specific BDADDR, if Secure
 * Connections are enabled by default.
 *
 * An existing override for a BDADDR can also be deleted.
 *
 * Bluestack maintains a list of BDADDR for which overrides have been set,
 * the size of the list is dictated by memory available.
 *****************************************************************************/

/*!
    @brief  Override the Secure Connections requirements for the specified
            remote device.

    @param  theAppTask The task to which the CFM message should be sent.
    @param  addr The Bluetooth Device Address of the remote host, for which
            the Secure Connection action shall be overridden / deleted, if
            there is an existing override.
    @param  override The override action to perform
            (see #sc_override_action_t).

    @return void

    A CL_DM_SECURE_CONNECTIONS_OVERRIDE_CFM message shall be sent to the
...
/*!
    @brief Request the maximum number of BDADDR that can be overridden using
    the ConnectionDmSecurConnectionsOverrideReq() function.

    @param theAppTask the task to which the CFM response should be sent.

    @return void

    The application should use this function to determine the maximum number
    of BDADDR that can be overridden for Secure Connections handling.

    A CL_DM_SECURE_CONNECTIONS_OVERRIDE_MAX_BDADDR_CFM message shall be
    sent to the application in response.
*/
