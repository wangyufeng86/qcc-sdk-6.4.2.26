        ...
        )
{
    ...
    if(accept_update)
    {   /* Accepted */
        prim->result = L2CAP_CONNECTION_PARAMETER_UPDATE_ACCEPT;
    }
    else
    {   /* An L2CAP Signal ID of 0 indicates that the Link Layer update 
           procedure has been used, use HCI status for LL rejection */           
        prim->result = (id == 0) ? 
