...
static void initialiseSpeech(void)
{
    ...
    if(AudioConfigIsVoiceTriggerEnabled() || (AudioConfigGetVaCaptureCodec() == va_capture_codec_msbc))
        amaSpeechSetAudioFormat(ama_audio_format_msbc);
    else if(AudioConfigGetVaCaptureCodec() == va_capture_codec_opus)
    {
        if(AudioConfigGetOpusCodecCapability()==audio_opus_codec_capability_32kbps)
        amaSpeechSetAudioFormat(ama_audio_format_opus_16khz_32kbps_cbr_0_20ms);
    else
        amaSpeechSetAudioFormat(ama_audio_format_opus_16khz_16kbps_cbr_0_20ms);
    }
