...
/***************************************************************************/
static void handleMsgMoreData(Source source)
{
    if(source)
    {
        if(amaValidStateToSendSpeech())
        {
            ...
            switch (codec)
            {
                ...
                AMA_DEBUG(("$"));
                all_sent = amaSendMsbcSpeechData(source);
                break;

                case va_capture_codec_opus:
                AMA_DEBUG(("%"));
                all_sent = amaSendOpusSpeechData(source);
                break;
            ...
            if (!all_sent)
            {
                sendMessageMoreData(AmaLibGetTask(), source, AMA_DATA_SEND_DELAY);
            }
        }
        else
        {
...
static void amaMessageHandler(Task task, MessageId id, Message message)
{
    ...
    switch(id)
    {
        case MESSAGE_MORE_DATA:
            handleMsgMoreData(((MessageMoreData *) message)->source);
            break;
...
void AmaLeDisconnected(void)
{
    AMA_DEBUG(("AMA AmaLeDisconnected\n"));
    amaDisconnect();
}
