...
static void hfpHandleRfcommConnectCfm(hfp_link_data* link, Sink sink, rfcomm_connect_status status)
{
    ...
    if(link)
    {
        switch(status)
        {
            ...
            default:
                if(link->identifier.sink && link->identifier.sink != sink)
                {
                    /* Connection cross-over: sink not matched with that in the link */
                    cfm_status = hfp_connect_failed_sink_not_matched;
                }
                else
                {
                    /* Generic failure */
