...
static void adaptToNewRate(uint32 sample_rate)
{
    ...
    Operator spdif_ttp_passthrough = ChainGetOperatorByRole(ctx->chain, spdif_ttp_passthrough_role);
    Operator spdif_iir_resampler = ChainGetOperatorByRole(ctx->chain, spdif_iir_resampler_role);
    ...
    OperatorsResamplerSetConversionRate(spdif_iir_resampler, sample_rate, AudioConfigGetMaxSampleRate());
