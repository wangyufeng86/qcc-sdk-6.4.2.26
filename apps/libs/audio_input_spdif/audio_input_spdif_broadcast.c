...
void audioInputSpdifBroadcastCreate(Task input_task, Task output_task, audio_input_context_t *ctx)
{
    ...
    params.forwarding.pcm.left_source = ChainGetOutput(ctx->chain, spdif_output_left);
    params.forwarding.pcm.right_source = ChainGetOutput(ctx->chain, spdif_output_right);
