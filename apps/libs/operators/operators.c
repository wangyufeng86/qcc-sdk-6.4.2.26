...
bool OperatorsFrameworkSetSampleRate(unsigned sample_rate)
{
    framework_sample_rate_param_t param;
    param.lsw = LOWORD(sample_rate);
    param.msw = HIWORD(sample_rate);

    return OperatorFrameworkConfigurationSet(FRAMEWORK_SAMPLE_RATE_PARAM, (uint16 *)&param, SIZEOF_OPERATOR_MESSAGE(param));
}
...
void OperatorsAecSetSameInputOutputClkSource(Operator op, bool enable)
{
...
void OperatorsAecEnableSidetonePath(Operator op, bool enable)
{
    standard_param_t aec_ref_config_param;

    aec_ref_config_param.id = AEC_REF_CONFIG_PARAM_INDEX;
    aec_ref_config_param.value = (enable) ?
            ((uint32)AEC_REF_CONFIG_PARAM_ENABLE_SIDETONE):((uint32)AEC_REF_CONFIG_PARAM_DEFAULT);
    operatorsSetSingleParameter(op, &aec_ref_config_param);
}

void OperatorsAecSetSidetoneGain(Operator op, uint32 exponent_value, uint32 mantissa_value)
{
