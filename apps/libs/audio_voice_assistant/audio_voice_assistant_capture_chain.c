...
#define OPUS_32KBPS_CAPABILITY_FRAME_SIZE 80
#define OPUS_16KBPS_CAPABILITY_FRAME_SIZE 40
...
static capability_id_t getEncoderCapabilityId(va_capture_codec_t capture_codec)
{
    switch(capture_codec)
    {
        ...
        case va_capture_codec_opus:
            return capability_id_opus_encoder;
...
static void configureEncoder(va_capture_codec_t codec)
{
    ...
    if(op_encoder)
    {
        ...
        case va_capture_codec_opus:
            /* Set Opus Encoder frame size as per application  confriguation for 16KBPS(40Bytes) or 32KBPS(80 Bytes) capability*/

            if(AudioConfigGetOpusCodecCapability()==audio_opus_codec_capability_32kbps)
                OperatorsSetOpusFrameSize(op_encoder,OPUS_32KBPS_CAPABILITY_FRAME_SIZE);
            else
                OperatorsSetOpusFrameSize(op_encoder,OPUS_16KBPS_CAPABILITY_FRAME_SIZE);
			break;
