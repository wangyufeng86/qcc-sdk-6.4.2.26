...
{
    DEBUG_LOGF("appHandleClDmModeChangeEvent, mode %d, interval %u, %x,%x,%lx",
               evt->mode, evt->interval,
               evt->bd_addr.nap, evt->bd_addr.uap, evt->bd_addr.lap);
...
void appTestSetAncToggleOnOff(void)
{
    DEBUG_LOG("appTestSetAncToggleOnOff");
    if (appSmIsOutOfCase())
    {
...
void appTestSetAncMode(anc_mode_t mode)
{
    DEBUG_LOG("appTestSetAncMode mode %d ", mode);
    if (appSmIsOutOfCase())
    {
        appKymeraAncSetModeSynchronizeWithPeer(mode);
    }
}

anc_mode_t appTestGetAncMode(void)
{
    DEBUG_LOG("appTestGetAncMode ");
    if (appSmIsOutOfCase() && appKymeraAncIsEnabled())
    {
        return appKymeraAncGetMode();
    }
    return 0xFF; /* invalid mode set */
}    

bool appTestSetAncGain(uint8 gain)
{
    DEBUG_LOG("appTestSetAncGain value %d ", gain);
    if (appSmIsOutOfCase())
    {
        return appKymeraAncSetGainSynchronizeWithPeer(gain);        
    }
    return FALSE;
}

uint8 appTestGetAncGain(void)
{
    DEBUG_LOG("appTestGetAncMode ");
    if (appSmIsOutOfCase() && appKymeraAncIsEnabled())
    {
        return appKymeraAncGetGain();
    }
    return 0x00;    
}
...
void appTestKymeraLeakthroughToggle(void)
{
    DEBUG_LOG("appTestKymeraLeakthroughToggle");
    if(appSmIsOutOfCase())
    {
        if(!appKymeraIsLeakthroughEnabled())
        {
            DEBUG_LOG("Enabling Leak through");
            appKymeraEnableLeakThrough();
        }
        else
        {
            DEBUG_LOG("Disabling Leak through");
...
void appTestKymeraSetLeakthroughMode(appKymeraLeakthroughMode mode)
{
    DEBUG_LOG("appTestKymeraSetLeakthroughMode");
    if (appSmIsOutOfCase())
    {
       appKymeraLeakthroughSetMode(mode);
    }
}

appKymeraLeakthroughMode appTestKymeraGetLeakthroughMode(void)
{
    DEBUG_LOG("appTestKymeraGetLeakthroughMode");

    return appKymeraLeakthroughGetMode();
}

bool appTestKymeraIsLeakthroughEnabled(void)
{
    DEBUG_LOG("appTestKymeraIsLeakthroughEnabled");
    return appKymeraIsLeakthroughEnabled();
}
