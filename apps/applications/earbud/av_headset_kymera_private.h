...
#define KICK_PERIOD_LEATHROUGH (KICK_PERIOD_SLOW)
...
/*! Bit in locks used to indicate tone in progess */
#define TONE_LOCK_BIT (1U)

/*! Bit in locks used to indicate A2DP chain starting */
#define STARTING_LOCK_BIT   (2U)

/*! Bit in locks used to indicate lock is set due to another audio chain in use */
#define BUSY_LOCK_BIT   (0x8000U)

/*!@{ \name Macros to set and clear bits in the lock. */
#define appKymeraSetToneLock(theKymera) \
    do { \
        (theKymera)->tone_lock |= TONE_LOCK_BIT; \
        (theKymera)->a2dp_lock |= TONE_LOCK_BIT; \
        (theKymera)->sco_lock |= TONE_LOCK_BIT;  \
        (theKymera)->anc_lock |= TONE_LOCK_BIT;  \
        (theKymera)->leakthrough_lock |= TONE_LOCK_BIT; \
    } while(0)

#define appKymeraClearToneLock(theKymera) \
    do { \
        (theKymera)->tone_lock &= ~TONE_LOCK_BIT; \
        (theKymera)->a2dp_lock &= ~TONE_LOCK_BIT; \
        (theKymera)->sco_lock &= ~TONE_LOCK_BIT;  \
...
#define appKymeraSetStartingLock(theKymera) \
    do { \
        (theKymera)->tone_lock |= STARTING_LOCK_BIT; \
        (theKymera)->a2dp_lock |= STARTING_LOCK_BIT; \
        (theKymera)->sco_lock |= STARTING_LOCK_BIT;  \
        (theKymera)->anc_lock |= STARTING_LOCK_BIT;  \
        (theKymera)->leakthrough_lock |= STARTING_LOCK_BIT; \
    } while(0)

#define appKymeraClearStartingLock(theKymera) \
    do { \
        (theKymera)->tone_lock &= ~STARTING_LOCK_BIT; \
        (theKymera)->a2dp_lock &= ~STARTING_LOCK_BIT; \
        (theKymera)->sco_lock &= ~STARTING_LOCK_BIT;  \
...
typedef enum kymera_operator_ucids
{
    ...
    UCID_AEC_UWB = 3,
#ifdef INCLUDE_AEC_LEAKTHROUGH
    UCID_AEC_LEAKTHROUGH_MODE_1 = 4,
    UCID_AEC_LEAKTHROUGH_MODE_2 = 5,
    UCID_AEC_LEAKTHROUGH_MODE_3 = 6,
    UCID_AEC_WB_LEAKTHROUGH_MODE_1 = 7,
    UCID_AEC_NB_LEAKTHROUGH_MODE_1 = 8,
    UCID_AEC_SWB_LEAKTHROUGH_MODE_1 = 9,
    UCID_AEC_UWB_LEAKTHROUGH_MODE_1 = 10,
    UCID_AEC_WB_LEAKTHROUGH_MODE_2 = 11,
    UCID_AEC_NB_LEAKTHROUGH_MODE_2 = 12,
    UCID_AEC_SWB_LEAKTHROUGH_MODE_2 = 13,
    UCID_AEC_UWB_LEAKTHROUGH_MODE_2 = 14,
    UCID_AEC_WB_LEAKTHROUGH_MODE_3 = 15,
    UCID_AEC_NB_LEAKTHROUGH_MODE_3 = 16,
    UCID_AEC_SWB_LEAKTHROUGH_MODE_3 = 17,
    UCID_AEC_UWB_LEAKTHROUGH_MODE_3 = 18,
#endif
    UCID_CVC_SEND = 0,
    UCID_CVC_RECEIVE = 0,
...
enum app_kymera_internal_message_ids
{
    ...
    /*! Internal ANC turn on indication used in peer synchronization */
    KYMERA_INTERNAL_ANC_ON,
    /*! Internal ANC turn off indication used in peer synchronization */
    KYMERA_INTERNAL_ANC_OFF,
    /*! Internal ANC Set Mode indication used in peer synchronization */
    KYMERA_INTERNAL_ANC_SET_MODE,
    /*! Internal ANC Set fine Gain indication used in peer synchronization */
    KYMERA_INTERNAL_ANC_SET_GAIN,
#ifdef INCLUDE_AEC_LEAKTHROUGH
    ...
    /*! Internal LEAKTHROUGH turn on indication used in peer synchronization */
    KYMERA_INTERNAL_LEAKTHROUGH_ON,
    /*! Internal LEAKTHROUGH turn off indication used in peer synchronization */
    KYMERA_INTERNAL_LEAKTHROUGH_OFF,
    /*! Internal LEAKTHROUGH Set mode indication used in peer synchronization */
    KYMERA_INTERNAL_LEAKTHROUGH_SET_MODE,
...
#define BASIC_PASS_BUFFER_SIZE (4096U)
#define SPC_BUFFER_SIZE   (1024U)
...
void appKymeraLeakthroughDisconnectAec(void);
...
bool appKymeraA2dpIsCodecAAC(void);
