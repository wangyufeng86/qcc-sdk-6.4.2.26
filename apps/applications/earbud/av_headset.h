...
/*! User PSKEY to store security key info for ANC production test mode*/
#define PS_SECURITY_KEY_CONFIG            (0)
...
typedef struct appTaskData
{
    ...
    appKymeraLeakthroughTaskData leakthrough_data_local; /*!< Task information for leakthrough specific data in peer synchronisation. */
#endif
    appKymeraAncTaskData  anc_data_local;       /*!< Local ANC specific task data used during peer signalling. */
...
/*! Get pointer to the current ANC data for states and mode set locally */
#define appGetAncDataLocal()   (&globalApp.anc_data_local)

#ifdef INCLUDE_AEC_LEAKTHROUGH
#define appGetLeakthroughDataLocal()    (&globalApp.leakthrough_data_local)
#endif
