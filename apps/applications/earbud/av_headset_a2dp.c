...
{    
    ...
    /* Clear role for this task */
    appLinkPolicyClearRole(&theInst->av_task);
}
...
/*! \brief Enter A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH

    The A2DP state machine has entered 'connected media role switch' state, this means
    that the A2DP media channel is connected and attempting to role-switch to be master
*/
static void appA2dpEnterConnectedMediaRoleSwitch(avInstanceTaskData *theInst)
{
    DEBUG_LOGF("appA2dpEnterConnectedMediaRoleSwitch(%p)", (void *)theInst);

    /* Attempt to role switch to be master */
    Sink sink = A2dpMediaGetSink(theInst->a2dp.device_id, theInst->a2dp.stream_id);

    /* Check if we're in a TWS+ sink */
    if (appA2dpIsSinkTwsCodec(theInst))
    {
        /* Request role switch to slave if connected to TWS+ handset, other wise
           don't care, just get current role */
        if (appDeviceIsTwsPlusHandset(&theInst->bd_addr))
            appLinkPolicySetSinkRole(&theInst->av_task, sink, hci_role_slave);
        else
            appLinkPolicySetSinkRole(&theInst->av_task, sink, hci_role_dont_care);
    }
    else
    {
        /* Non TWS codec so role switch to be master */
        appLinkPolicySetSinkRole(&theInst->av_task, sink, hci_role_master);
    }

    /* Set locks */
    appA2dpSetRoleSwitchLockBit(theInst);
    appA2dpSetTransitionLockBit(theInst);
}

/*! \brief Exit A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH

    The A2DP state machine has exited 'connected media role switch' state, this means
    that the role switch procedure has finished.
*/
static void appA2dpExitConnectedMediaRoleSwitch(avInstanceTaskData *theInst)
{
    DEBUG_LOGF("appA2dpExitConnectedMediaRoleSwitch(%p)", (void *)theInst);

    /* Clear locks */
    appA2dpClearRoleSwitchLockBit(theInst);
    appA2dpClearTransitionLockBit(theInst);
}
...
static void appA2dpSetState(avInstanceTaskData *theInst, avA2dpState a2dp_state)
{
    ...
    switch (a2dp_old_state)
    {
        ...
        case A2DP_STATE_CONNECTING_MEDIA_REMOTE_SYNC:
            ...
            break;
        case A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH:
            appA2dpExitConnectedMediaRoleSwitch(theInst);
            break;
    ...
    switch (a2dp_state)
    {
        ...
        case A2DP_STATE_CONNECTING_MEDIA_REMOTE_SYNC:
            ...
            break;
        case A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH:
            appA2dpEnterConnectedMediaRoleSwitch(theInst);
            break;
        case A2DP_STATE_CONNECTED_MEDIA_STREAMING:
    ...
    switch (appA2dpGetState(theInst))
    {
        case A2DP_STATE_CONNECTING_MEDIA_LOCAL:
        {
            ...
            if (cfm->status == a2dp_success)
            {
                ...
                appA2dpSetState(theInst, A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH);
            }
            else
            {
                /* Move to 'connected signalling' state */
                                                 ...
                                                 const A2DP_MEDIA_START_IND_T *ind)
{
    ...
    /* Delay handling this message if role switch in progress */
    if (appA2dpGetRoleSwitchLock(theInst))
    {
        DEBUG_LOGF("appA2dpHandleA2dpMediaStartIndication(%p) state(0x%x) role switch pending!!",
                    (void *)theInst, appA2dpGetState(theInst));

        MAKE_AV_MESSAGE(A2DP_MEDIA_START_IND);
        *message = *ind;
        MessageSendConditionally(&theInst->av_task, A2DP_MEDIA_START_IND, message, &appA2dpGetRoleSwitchLock(theInst));
        return;
    }
    ...
    switch (appA2dpGetState(theInst))
    {
        ...
        case A2DP_STATE_CONNECTED_MEDIA_STREAMING_MUTED:
        {
            ...
            {
                DEBUG_LOG("appA2dpHandleA2dpMediaStartIndication shouldn't start .. but we will try");
            }
...
static void appA2dpHandleLinkPolicyRoleSwitchConfirm(avInstanceTaskData *theInst,
                                                     const APP_LP_ROLE_SWITCH_CFM_T *cfm)
{
    if (cfm->role == hci_role_master)
    {
        DEBUG_LOGF("appA2dpHandleLinkPolicyRoleSwitchConfirm(%p), master",
                   (void *)theInst);
    }
    else
    {
        DEBUG_LOGF("appA2dpHandleLinkPolicyRoleSwitchConfirm(%p), slave",
                   (void *)theInst);
    }
    ...
    switch (appA2dpGetState(theInst))
    {
        case A2DP_STATE_CONNECTED_MEDIA_ROLE_SWITCH:
        {
            if (!theInst->a2dp.suspend_state && appAvInstanceShouldStartMediaStreamingOnMediaOpenCfm(theInst))
                appA2dpSetState(theInst, A2DP_STATE_CONNECTED_MEDIA_STARTING_LOCAL_SYNC);
            else
                appA2dpSetState(theInst, A2DP_STATE_CONNECTED_MEDIA_SUSPENDED);
        }
...
void appA2dpInstanceHandleMessage(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case APP_LP_ROLE_SWITCH_CFM:
            appA2dpHandleLinkPolicyRoleSwitchConfirm(theInst, (APP_LP_ROLE_SWITCH_CFM_T *)message);
            return;
    }
