...
static void appHfpEnterConnected(void)
{
    ...
    if (appDeviceIsTwsPlusHandset(appHfpGetAgBdAddr()))
        appLinkPolicySetSinkRole(appGetHfpTask(), appGetHfp()->slc_sink, hci_role_slave);
    else
        appLinkPolicySetSinkRole(appGetHfpTask(), appGetHfp()->slc_sink, hci_role_master);
...
static void appHfpExitConnected(void)
{
    ...
    /* Don't care about role anymore */
    appLinkPolicyClearRole(appGetHfpTask());
...
static void appHfpHandleHfpRingIndication(const HFP_RING_IND_T *ind)
{
    ...
    */
    ...
    DEBUG_LOGF("appHfpHandleHfpRingIndication ring_fwd status %d (%d %d %d)",
              ring_fwd,
              appConfigScoForwardingEnabled(),
              appDeviceIsPeerRingForwardSupported(),
              appDeviceIsPeerConnected());
...
static void appHfpHandleLinkPolicyRoleSwitchConfirm(const APP_LP_ROLE_SWITCH_CFM_T *cfm)
{
    DEBUG_LOG("appHfpHandleLinkPolicyRoleSwitchConfirm, role %u", cfm->role);
}
...
static void appHfpHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case APP_LP_ROLE_SWITCH_CFM:
            appHfpHandleLinkPolicyRoleSwitchConfirm((APP_LP_ROLE_SWITCH_CFM_T *)message);
            return;
    }
