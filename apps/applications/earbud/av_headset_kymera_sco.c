...
void appKymeraHandleInternalScoStop(void)
{
    ...
    if (appKymeraGetState() != KYMERA_STATE_SCO_ACTIVE)
    {
        ...
        DEBUG_LOG("appKymeraHandleInternalScoStop, not stopping as not in active state");
        return;
    }
    ...
    switch (appKymeraGetState())
    {
        ...
        case KYMERA_STATE_SCO_SLAVE_ACTIVE:
        {
            ...
            if (GET_OP_FROM_CHAIN(aec_op, appKymeraGetScoChain(), OPR_SCO_AEC))
            {
                ...
                DEBUG_LOGF("appKymeraHandleInternalScoMicMute, no AEC reference operator found");

        }
        break;

        default:
            DEBUG_LOGF("appKymeraHandleInternalScoMicMute, ignoring in state %u", appKymeraGetState());
            break;
    }
}
