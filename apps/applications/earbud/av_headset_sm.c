...
{
    DEBUG_LOG("appSmHandleConnRulesScoTimeout");
...
static void appSmHandleConnRulesPeerLinkSniff(const CONN_RULES_PEER_LINK_STATUS_T* pls)
{
    DEBUG_LOGF("appSmHandleConnRulesPeerLinkSniff, in_case %u, streaming %u",
               pls->in_case, pls->streaming);
    appLinkPolicyUpdatePeerPowerTable(pls->in_case, pls->streaming);
}
...
static void appSmHandleAvStreamingActiveInd(void)
{
    ...
    /* At least one Earbud is streaming, so update sniff parameters */
    appLinkPolicyUpdatePeerPowerTable(EBS_IN_CASE_UNKNOWN, EBS_STREAMING);
...
void appSmHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CONN_RULES_PAGE_SCAN_UPDATE:
            ...
            break;
        case CONN_RULES_PEER_LINK_STATUS:
            appSmHandleConnRulesPeerLinkSniff((CONN_RULES_PEER_LINK_STATUS_T*)message);
            break;
        ...
        case CONN_RULES_PEER_SYNC_LEAKTHROUGH_DATA:
        {
            DEBUG_LOG("SyncLeakthroughPeerData from remote peer onto local when back to out of case state");
            appKymeraLeakthroughTaskData *data = appGetLeakthroughDataLocal();
            appKymeraHandleLeakThroughSynchronization(GET_LEAKTHROUGH_ON_STATE(data->leakthrough_state_on), KYMERA_LEAKTHROUGH_NO_DELAY);
            /* Mode can be set in LEAKTHROUGH_ON state only */
            if (appKymeraIsLeakthroughEnabled())
            {
                appKymeraHandleLeakThroughSynchronization(GET_LEAKTHROUGH_MODE(data->leakthrough_mode), KYMERA_LEAKTHROUGH_NO_DELAY);
            }
        }
        break;
        case CONN_RULES_AEC_LEAKTHROUGH_DISABLE:
            appKymeraDisableLeakThrough();
            break;
#else
        case CONN_RULES_PEER_SYNC_ANC_DATA:
        {
            DEBUG_LOG("SyncAncPeerData from remote peer onto local when back to out of case state");
            appKymeraAncTaskData *data = appGetAncDataLocal();
            appKymeraAncProcessCommand(GET_ANC_ON_STATE(data->anc_state_on), KYMERA_ANC_NO_DELAY);
            /* Mode and Gain can be set in ANC_ON state only */
            if (GET_ANC_ON_STATE(data->anc_state_on))
            {         
                appKymeraAncProcessCommand(GET_ANC_MODE(data->anc_mode), KYMERA_ANC_NO_DELAY);
                appKymeraAncProcessCommand(GET_ANC_GAIN_EVENT_CMD(data->anc_gain), KYMERA_ANC_NO_DELAY);
            }
        }
        break;
#endif
