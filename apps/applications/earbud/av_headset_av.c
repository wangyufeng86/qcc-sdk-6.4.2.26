...
static void appAvInstanceHandleMessage(Task task, MessageId id, Message message)
{
    ...
    if (id >= AV_INTERNAL_AVRCP_BASE && id < AV_INTERNAL_AVRCP_TOP)
        ...
        appA2dpInstanceHandleMessage(theInst, id, message);
    else if (id >= APP_LP_MESSAGE_BASE && id < APP_LP_MESSAGE_TOP)
        appA2dpInstanceHandleMessage(theInst, id, message);
    else
    {
        switch (id)
...
{
    bdaddr bd_addr;
    bool a2dp_disc_req = FALSE;
    bool avrcp_disc_req = FALSE;

    DEBUG_LOG("appAvDisconnectPeer");

    /* Get peer device address */
    if (appDeviceGetPeerBdAddr(&bd_addr))
    {
        avInstanceTaskData *av_inst = appAvInstanceFindFromBdAddr(&bd_addr);
        a2dp_disc_req = appAvA2dpDisconnectRequest(av_inst);
        avrcp_disc_req = appAvAvrcpDisconnectRequest(&av_inst->av_task, av_inst);

        DEBUG_LOGF("appAvDisconnectPeer, a2dp_disc_req %d, avrcp_disc_req %d", a2dp_disc_req, avrcp_disc_req);
    }

    return (a2dp_disc_req || avrcp_disc_req);
}
...
bool appAvDisconnectHandset(void)
{
    ...
    DEBUG_LOG("appAvDisconnectHandset");
    ...
    if (appDeviceGetHandsetBdAddr(&bd_addr))
    {
        ...
        DEBUG_LOGF("appAvDisconnectHandset, a2dp_disc_req %d, avrcp_disc_req %d", a2dp_disc_req, avrcp_disc_req);
    }
