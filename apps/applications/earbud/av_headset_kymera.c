                         ...
                         bool interruptible, uint16 *client_lock, uint16 client_lock_mask)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_TONE_PROMPT_PLAY, message, &theKymera->tone_lock);
...
{
    kymeraTaskData *theKymera = appGetKymera();
    DEBUG_LOGF("appKymeraA2dpStart, seid %u, a2dp_lock %u", codec_settings->seid, theKymera->a2dp_lock);

#ifdef INCLUDE_AEC_LEAKTHROUGH
    appKymeraLeakthroughStopChainIfRunning();
#endif
...
void appKymeraA2dpStop(uint8 seid, Source source)
{
    ...
    MessageSendConditionally(&theKymera->task, mid, message, &theKymera->a2dp_lock);
}
...
void appKymeraA2dpSetVolume(uint16 volume)
{
    ...
    message->volume = volume;
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_A2DP_SET_VOL, message, &theKymera->a2dp_lock);
}
...
void appKymeraScoStartForwarding(Sink forwarding_sink, bool enable_mic_fwd)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_START_FORWARDING_TX, message, &theKymera->sco_lock);
}
...
void appKymeraScoStopForwarding(void)
{
    ...
    if (!appKymeraHandleInternalScoForwardingStopTx())
        MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_STOP_FORWARDING_TX, NULL, &theKymera->sco_lock);
}
                                    ...
                                    uint16 volume, uint8 pre_start_delay, bool conditionally)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_START, message, conditionally ? &theKymera->sco_lock : NULL);
}
...
void appKymeraScoStop(void)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_STOP, NULL, &theKymera->sco_lock);
}
...
void appKymeraScoSlaveStartHelper(Source link_source, uint8 volume, const appKymeraScoChainInfo *info, uint16 delay)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_SLAVE_START, message, &theKymera->sco_lock);
}
...
void appKymeraScoSlaveStop(void)
{
    ...
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCOFWD_RX_STOP, NULL, &theKymera->sco_lock);
}
...
void appKymeraScoSetVolume(uint8 volume)
{
    ...
    message->volume = volume;
    MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_SCO_SET_VOL, message, &theKymera->sco_lock);
}
...
void appKymeraScoUseLocalMic(void)
{
    ...
    if (appDeviceIsPeerMicForwardSupported())
    {
        ...
        MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_MICFWD_LOCAL_MIC, NULL, &theKymera->sco_lock);
    }
}
...
void appKymeraScoUseRemoteMic(void)
{
    ...
    if (appDeviceIsPeerMicForwardSupported())
    {
        ...
        MessageSendConditionally(&theKymera->task, KYMERA_INTERNAL_MICFWD_REMOTE_MIC, NULL, &theKymera->sco_lock);
    }
}
...
static void kymera_msg_handler(Task task, MessageId id, Message msg)
{
    ...
    switch (id)
    {
        ...
        {
            const KYMERA_INTERNAL_A2DP_STOP_T *m = (const KYMERA_INTERNAL_A2DP_STOP_T *)msg;
            appKymeraHandleInternalA2dpStop(m);
        }
        break;
        ...
        case KYMERA_INTERNAL_ANC_SET_MODE:
        {
            appKymeraAncTaskData *data = appGetAncDataLocal();
            appKymeraAncSetMode(data->anc_mode);
        }
        break;

        case KYMERA_INTERNAL_ANC_SET_GAIN:
        {
             appKymeraAncTaskData *data = appGetAncDataLocal();
             appKymeraAncSetGain(data->anc_gain);
        }
        break;
        ...
        break;
        case KYMERA_INTERNAL_LEAKTHROUGH_ON:
            appKymeraEnableLeakThroughLocally();
             break;

        case KYMERA_INTERNAL_LEAKTHROUGH_OFF:
            appKymeraDisableLeakThroughLocally();
            break;

        case KYMERA_INTERNAL_LEAKTHROUGH_SET_MODE:
        {
           appKymeraLeakthroughTaskData *data = appGetLeakthroughDataLocal();
           appKymeraLeakthroughSetModeLocally(data->leakthrough_mode);
        }
        break;
    #endif
        default:
...
void appKymeraInit(void)
{
    ...
    theKymera->tone_lock = theKymera->sco_lock = theKymera->a2dp_lock = theKymera->anc_lock = theKymera->leakthrough_lock = 0;
