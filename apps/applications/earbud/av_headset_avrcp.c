...
static void appAvrcpEnterDisconnected(avInstanceTaskData *theInst)
{
    ...
    {
        Task iter_task = 0;
        while (appTaskListIterate(theInst->avrcp.client_list, &iter_task))
            appTaskListRemoveTask(theInst->avrcp.client_list, iter_task);
    }
...
{
    return theInst->avrcp.client_list ? appTaskListRemoveTask(theInst->avrcp.client_list, client_task) : FALSE;
}
