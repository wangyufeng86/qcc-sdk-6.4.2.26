    ...
    switch (appPairingGetState(thePairing))
    {
        ...
        case PAIRING_STATE_PEER_SCO_FWD_SDP_SEARCH:
        {
            ...
            else
            {
                ...
                DEBUG_LOG("appHandleClSdpServiceSearchAttributeCfm, failed peer SCO fowrard SDP search, retrying");
                ...
                {
                    /* Send message to retry after 1 second */
                    MessageSendLater(&thePairing->task, PAIRING_INTERNAL_PEER_SCO_FWD_SDP_SEARCH, NULL, D_SEC(1));
                }
                else
                {
                    /* Send confirmation to main task */
...
static void appHandleInternalPeerScoFwdSdpSearch(pairingTaskData *thePairing)
{
    DEBUG_LOG("appHandleInternalPeerScoFwdSdpSearch");
...
static void appPairingHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case PAIRING_INTERNAL_PEER_SCO_FWD_SDP_SEARCH:
            appHandleInternalPeerScoFwdSdpSearch(thePairing);
            break;
