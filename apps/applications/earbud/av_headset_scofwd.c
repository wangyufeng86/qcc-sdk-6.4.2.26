...
static void appScoFwdEnterConnectedActiveSendRoleSwitch(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
    DEBUG_LOG("appScoFwdEnterConnectedActiveSendRoleSwitch");
    
    appLinkPolicySetSinkRole(&theScoFwd->task, theScoFwd->link_sink, hci_role_master);
}
...
static void appScoFwdSetState(scoFwdState new_state)
{
    ...
    switch (new_state)
    {
        ...
        case SFWD_STATE_CONNECTED_ACTIVE_SEND_ROLE_SWITCH:
            appScoFwdEnterConnectedActiveSendRoleSwitch();
            break;        
...
void appScoFwdCancelNotifyIncomingMicSink(void)
{
    MessageCancelFirst(appGetScoFwdTask(),SFWD_INTERNAL_MIC_CHAIN_DETAILS);
}
...
static void appScoFwdHandleEnableForwarding(void)
{
    ...
    if (appScoFwdGetState() == SFWD_STATE_CONNECTED_ACTIVE)
    {
        ...
        appScoFwdSetState(SFWD_STATE_CONNECTED_ACTIVE_SEND_ROLE_SWITCH);
    }
}
...
static void appScoFwdHandleDisableForwarding(void)
{
    ...
    if ((appScoFwdGetState() == SFWD_STATE_CONNECTED_ACTIVE_SEND) || 
        (appScoFwdGetState() == SFWD_STATE_CONNECTED_ACTIVE_SEND_ROLE_SWITCH))
        appScoFwdSetState(SFWD_STATE_CONNECTED_ACTIVE);
}
...
static void appScoFwdHandleLinkPolicyRoleSwitchConfirm(const APP_LP_ROLE_SWITCH_CFM_T *cfm)
{
    DEBUG_LOGF("appScoFwdHandleLinkPolicyRoleSwitchConfirm, state %u, role %u", appScoFwdGetState(), cfm->role);
 
    if (appScoFwdGetState() == SFWD_STATE_CONNECTED_ACTIVE_SEND_ROLE_SWITCH)
    {
...
static void appScoFwdHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case APP_LP_ROLE_SWITCH_CFM:
            appScoFwdHandleLinkPolicyRoleSwitchConfirm((APP_LP_ROLE_SWITCH_CFM_T *)message);
            break;
