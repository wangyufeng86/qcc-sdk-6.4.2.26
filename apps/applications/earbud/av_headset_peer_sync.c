...
static void appPeerSyncHandlePeerSigMsgChannelTxCfm(const PEER_SIG_MSG_CHANNEL_TX_CFM_T *cfm)
{
    ...
    else
    {
        ...
        /* Set event to indicate peer sync has failed, this will cause any deferred rules to
         * re-run */
        appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_PEER_SYNC_FAILED);
    }
...
bool appPeerSyncIsPeerStreaming(void)
{
    peerSyncTaskData* ps = appGetPeerSync();

    if (appPeerSyncIsComplete())
        return ps->peer_a2dp_streaming;
