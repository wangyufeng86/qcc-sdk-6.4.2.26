...
static bool appPowerAllClientsHaveFlagCleared(uint32 flag)
{
    ...
    {
        while (appTaskListIterateWithDataRaw(clients, &next_task, (void **)&data))
        {
            if (*data & flag)
            {
                return FALSE;
...
{
    uint32 *data = NULL;
    if (appTaskListGetDataForTaskRaw(appPowerGetClients(), task, (void **)&data))
    {
        *data |= flag;
        return TRUE;
    }
...
void appPowerInit(void)
{
    ...
    thePower->clients = appTaskListWithDataInit(sizeof(uint32));
