...
typedef enum chain_operator_roles
{
    ...
    /*Below Opearator roles are required only for AEC leakthrough operations.*/
    /*! AEC Operator */
    OPR_LEAKTHROUGH_AEC,

    /*! Switched passthrough for AEC mic out path */
    OPR_LEAKTHROUGH_INPUT_SPC,

    /*! Basic pass for AEC spk input path */
    OPR_LEAKTHROUGH_OUTPUT_PASS,

    /*! Switch pass through consumer for A2DP leakthrough */
    OPR_LEAKTHROUGH_SPC,

    /*! Basic pass for SPC to avoid vol_ctrl buffering issue */
    OPR_LEAKTHROUGH_VOL_LATECY_BUFFER,
...
typedef enum chain_endpoint_roles
{
    ...
    /* Below endpoint roles are required only for AEC leakthrough opearations.*/
    /*! Input to the standalone leakthrough AEC ref chain */
    EPR_LEAKTHROUGH_IN,

    /*! Output from the standalone leakthrough AEC ref chain */
    EPR_LEAKTHROUGH_OUT,

    /*! Output from the A2DP leakthrouh AEC Ref chain */
    EPR_A2DP_LEAKTHROUGH_AEC_OUT,

    /*! Output from the A2DP SPC chain */
    EPR_A2DP_SPC_OUT,
