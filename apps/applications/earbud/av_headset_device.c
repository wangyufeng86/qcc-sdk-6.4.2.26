...
static void appDeviceUpdateHandsetConnected(void)
{
    ...
    /* Iterate through handset devices, checking if any are connected */
    while (appDeviceGetHandsetAttributes(&handset_addr, &attr, &index))
    {
        if (appConManagerIsConnected(&handset_addr))
        {
            theDevice->handset_connected = TRUE;
            return;
        }
    }
    theDevice->handset_connected = FALSE;
}
...
static void appDeviceUpdateCache(deviceTaskData *theDevice, appDeviceAttributes *attributes, const bdaddr *bd_addr)
{
    if (attributes->tws_version != DEVICE_TWS_UNKNOWN)
    {
        ...
        else if (attributes->type == DEVICE_TYPE_HANDSET && !BdaddrIsZero(bd_addr))
        {
            ...
            appDeviceUpdateHandsetConnected();
...
static void appDeviceHandleConManagerConnectionInd(CON_MANAGER_CONNECTION_IND_T* ind)
{
    ...
    if (!ind->ble)
    {
        if (appDeviceIsHandset(&ind->bd_addr))
        {
            ...
            DEBUG_LOGF("appDeviceHandleConManagerConnectionInd, handset %x,%x,%lx, connected %d, status %d",
                       ind->bd_addr.nap, ind->bd_addr.uap, ind->bd_addr.lap, ind->connected, ind->reason);

            DEBUG_LOGF("appDeviceHandleConManagerConnectionInd, current handset, handset_connected was %d",
                       theDevice->handset_connected);

            appDeviceUpdateHandsetConnected();
        }
        else if (appDeviceIsPeer(&ind->bd_addr))
        {
            DEBUG_LOGF("appDeviceHandleConManagerConnectionInd, peer %x,%x,%lx, connected %d, status %d",
                       ind->bd_addr.nap, ind->bd_addr.uap, ind->bd_addr.lap, ind->connected, ind->reason);
            theDevice->peer_connected = ind->connected;
        }
        else
        {
            DEBUG_LOGF("appDeviceHandleConManagerConnectionInd, unknown BR/EDR %x,%x,%lx, connected %d, status %d",
                       ind->bd_addr.nap, ind->bd_addr.uap, ind->bd_addr.lap, ind->connected, ind->reason);
        }
    }
    else
    {
        DEBUG_LOGF("appDeviceHandleConManagerConnectionInd unknown BLE %x,%x,%lx, connected %d, status %d",
                   ind->bd_addr.nap, ind->bd_addr.uap, ind->bd_addr.lap, ind->connected, ind->reason);
        return;
    }
...
void appDeviceSetAttributes(const bdaddr *bd_addr, appDeviceAttributes *attributes)
{
    ...
    /* Update cache if it is the currently connected handset or peer */
