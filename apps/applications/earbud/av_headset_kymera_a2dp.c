...
static void appKymeraA2dpCommonStop(Source source)
{
...
#ifdef INCLUDE_AEC_LEAKTHROUGH
    PanicNull(theKymera->chain_leakthrough_handle);
...
#ifdef INCLUDE_AEC_LEAKTHROUGH
    if(theKymera->chain_leakthrough_handle)
    {
    ...
    if(theKymera->chainu.output_vol_handle)
    {
        DEBUG_LOG("Destroying the output_volume_chain");
    ...
    if(theKymera->chain_leakthrough_handle)
    {
        DEBUG_LOG("Destroying the output leakthorough chain");
        ChainDestroy(theKymera->chain_leakthrough_handle);
        theKymera->chain_leakthrough_handle = NULL;
    }
    appKymeraUpdateLeakthroughMicSampleRate(MIC_RATE);
#endif
    ...
    if (appA2dpIsSeidNonTwsSink(seid) || appA2dpIsSeidTwsSink(seid))
    {
        switch (appKymeraGetState())
        {
            ...
            case KYMERA_STATE_A2DP_STREAMING:
                ...
                appKymeraSetState(KYMERA_STATE_IDLE);
                #ifdef INCLUDE_AEC_LEAKTHROUGH
                    appKymeraLeakthroughResumeChainIfSuspended();
                #endif
            break;
