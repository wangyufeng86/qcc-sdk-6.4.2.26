        ...
        else 
        {
            ...
            if (handset && theConMgr->handset_connect_allowed)
            {
                ...
                if ((ind->protocol_id == protocol_l2cap) && (ind->channel == 1))
                {
                    ...
                    DEBUG_LOG("appConManagerHandleClSmAuthoriseIndication, ALLOW handset SDP (bredr)");
                    authorise = TRUE;
                }
                else if (BdaddrIsSame(&handset_bd_addr, &ind->bd_addr) || !appConManagerIsConnected(&handset_bd_addr))
                {
