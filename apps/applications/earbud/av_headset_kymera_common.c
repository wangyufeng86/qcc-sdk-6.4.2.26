...
void appKymeraSetState(appKymeraState state)
{
    ...
    /* If back into idle state, clear the busy bit in all 3 audio chain locks */
    if (state == KYMERA_STATE_IDLE)
    {
        theKymera->a2dp_lock &= ~BUSY_LOCK_BIT;
        theKymera->sco_lock &= ~BUSY_LOCK_BIT;
        theKymera->anc_lock &= ~BUSY_LOCK_BIT;
#ifdef INCLUDE_AEC_LEAKTHROUGH
        theKymera->leakthrough_lock &= ~BUSY_LOCK_BIT;
#endif
    }
#ifdef INCLUDE_AEC_LEAKTHROUGH
    else if(state == KYMERA_STATE_STANDALONE_LEAKTHROUGH){
        theKymera->a2dp_lock &= ~0x8000;
        theKymera->sco_lock &= ~0x8000;
        theKymera->anc_lock &= ~0x8000;
    }
#endif
    else
    {
        /* If we're not in the A2DP starting or streaming states, or idle (see above) then A2DP is locked out */
        if ((state < KYMERA_STATE_A2DP_STARTING_A) || (state > KYMERA_STATE_A2DP_STREAMING_WITH_FORWARDING))
            theKymera->a2dp_lock |= BUSY_LOCK_BIT;
        else
            theKymera->a2dp_lock &= ~BUSY_LOCK_BIT;

        /* If we're not in the SCO active states or idle (see above) then SCO is locked out */
        if ((state < KYMERA_STATE_SCO_ACTIVE) || (state > KYMERA_STATE_SCO_SLAVE_ACTIVE))
            theKymera->sco_lock |= BUSY_LOCK_BIT;
        else
            theKymera->sco_lock &= ~BUSY_LOCK_BIT;

        /* If we're not in the ANC tuning state or idle (see above) then ANC tuning is locked out */
        if ((state != KYMERA_STATE_ANC_TUNING))
            theKymera->anc_lock |= BUSY_LOCK_BIT;
        else
            theKymera->anc_lock &= ~BUSY_LOCK_BIT;

#ifdef INCLUDE_AEC_LEAKTHROUGH
        if ((state != KYMERA_STATE_STANDALONE_LEAKTHROUGH))
            theKymera->leakthrough_lock |= BUSY_LOCK_BIT;
        else
            theKymera->leakthrough_lock &= ~BUSY_LOCK_BIT;
#endif
        ...
        case KYMERA_STATE_STANDALONE_LEAKTHROUGH:
        {
            ...
            else
            {
                ...
                switch (theKymera->a2dp_seid)
                {
                    ...
                    {
                        DEBUG_LOG("appKymeraConfigureDspPowerMode : AEC_LEAKTHROUGH_ENABLED");
                        cconfig.active_mode = AUDIO_DSP_BASE_CLOCK;
                        mode = AUDIO_POWER_SAVE_MODE_1;
                    }
                    break;
...
#ifdef INCLUDE_AEC_LEAKTHROUGH
static void appKymeraCreateAecOutputChain(uint32 rate, uint16 aec_terminal_buffer_size)
{
    kymeraTaskData *theKymera = appGetKymera();

    theKymera->chain_leakthrough_handle = ChainCreate(&chain_leakthrough_aec_config);
    appKymeraLeakthroughConfigureAecChain(theKymera->chain_leakthrough_handle,rate,aec_terminal_buffer_size);

    ChainConnect(theKymera->chain_leakthrough_handle);

}

static void appKymeraConnectVolCtrlToAec(void)
{
    #define AEC_REF_SPK_L_INPUT_TERMINAL (0U)
    #define VOL_CTRL_OUTPUT_TERMINAL (0U)

    kymeraTaskData *theKymera = appGetKymera();
    Operator leak_aec = ChainGetOperatorByRole(theKymera->chain_leakthrough_handle,OPR_LEAKTHROUGH_AEC);
    Operator vol_ctrl = ChainGetOperatorByRole(theKymera->chainu.output_vol_handle,OPR_VOLUME_CONTROL);

    StreamConnect(StreamSourceFromOperatorTerminal(vol_ctrl,VOL_CTRL_OUTPUT_TERMINAL),StreamSinkFromOperatorTerminal(leak_aec,AEC_REF_SPK_L_INPUT_TERMINAL));
}

static void appKymeraConnectAecOuputToDac(uint32 rate)
{
    kymeraTaskData *theKymera = appGetKymera();

    Sink DAC_SNK_L = (Sink)PanicFalse(StreamAudioSink(AUDIO_HARDWARE_CODEC, AUDIO_INSTANCE_0, AUDIO_CHANNEL_A));
    PanicFalse(SinkConfigure(DAC_SNK_L, STREAM_CODEC_OUTPUT_RATE, rate));
    PanicFalse(SinkConfigure(DAC_SNK_L, STREAM_RM_ENABLE_DEFERRED_KICK, 0));

    ChainConnectOutput(theKymera->chain_leakthrough_handle, DAC_SNK_L,EPR_A2DP_LEAKTHROUGH_AEC_OUT);
}
#endif

#ifndef INCLUDE_AEC_LEAKTHROUGH
static void appKymeraConnectVolOutputToDac(uint32 rate)
{
    ...
    DAC_SNK_L = StreamAudioSink(appConfigLeftAudioHardware(), appConfigLeftAudioInstance(), appConfigLeftAudioChannel());
    PanicFalse(SinkConfigure(DAC_SNK_L, STREAM_CODEC_OUTPUT_RATE, rate));
    PanicFalse(SinkConfigure(DAC_SNK_L, STREAM_RM_ENABLE_DEFERRED_KICK, 0));

    ChainConnectOutput(theKymera->chainu.output_vol_handle,DAC_SNK_L,EPR_SOURCE_MIXER_OUT);
}
#endif
                                ...
                                unsigned buffer_size, uint8 volume)
{
    ...
    /*Default terminal buffer size of AEC ref operator corresponding to 15ms*/
    uint16 aec_terminal_buffer_size=15;

    if(appKymeraA2dpIsCodecAAC())
    {
        /*In case of AAC codec AEC ref operator terminal buffer size should correspond to 30ms*/
        aec_terminal_buffer_size = 30;
    }
    /* Create output volume chain */
    appKymeraCreateOutputVolumeChain(rate,kick_period,buffer_size,volume);
    appKymeraCreateAecOutputChain(rate,aec_terminal_buffer_size);
    appKymeraConnectVolCtrlToAec();
    appKymeraConnectAecOuputToDac(rate);

#else
    appKymeraCreateOutputVolumeChain(rate, kick_period, buffer_size,volume);
    appKymeraConnectVolOutputToDac(rate);
#endif
